...
del prem[0]
# berisi pilihan jawaban untuk setiap premis
choices = [[] for i in range(len(prem))]
...
for ind in poke.index:
    for col in poke.columns.values:
        if pd.notna(poke[col][ind]) and col != 'nama':
            attr[ind].append(
                [prem.index(col), choices[prem.index(col)].index(poke[col][ind])])
...
pcs = [['FR' for y in range(len(poke['attr'][x]))]
       for x in range(len(poke))]  # premise clause status
            ...
            if 'U' in rs[i] and 'A' in rs[i]:
                ...
                for j in range(len(pcs[recent])):
                    if pcs[recent][j] == 'FR':  # query user for value
                        ...
                        # if no response atau gaada
                        while valInput not in choices[query] or not valid:
                            print("Tidak menemukan data mohon coba lagi")
                            window = tk.Tk()
                            updateGUI(prem[query], optPil,
                                      "Tidak menemukan data mohon coba lagi")
                            # valInput = input()
                            # cek keberadaan itu di yang masih aktif
                            if valInput in choices[query]:
                                for i in range(len(rs)):
